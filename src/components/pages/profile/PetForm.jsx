...
const PetForm = ({ user_id }) => {
    ...
    const getPetBreedData = () => {
        let payload = {};
        pet.pet_type && (payload.master_type_id = pet.pet_type)
        PetService.getPetBreeds(payload).then((r) => {
            if (r.data.status) {
    ...
    return (
        <>
            <div>
                <button onClick={openPopup} className="justify-center items-stretch w-[156px] border-[color:var(--Secondary-1,#5281A2)] flex gap-2 px-8 py-3 rounded-[86px] border-2 border-solid">
                    Add Pet
                </button>
            </div>
            <Popup isOpen={isOpen} onClose={closePopup} hideClose>
                <div className='bg-primary3 w-[430px] flex flex-col justify-center items-center pt-10 '>
                    ...
                    <div className='w-[80%] mt-5'>
                        <select value={pet.pet_type} onChange={(e) => handlePetTypeChange(e)} class="rounded-lg px-4 py-2 w-full border-2 border-secondary2 text-lg text-primary">
                    ...
                    {pet.pet_type && <div className='w-[80%] mt-5'>
                        <select value={pet.breed} onChange={(e) => setPet({ ...pet, breed: e.target.value })} class="rounded-lg px-4 py-2 w-full border-2 border-secondary2 text-lg text-primary">
                            <option value="" disabled selected>Select your pet</option>
                        ...
                        pet.breed && <>
                            <div className='flex justify-between w-[80%] m-auto mt-4'>
                                <div className='w-[45%] flex flex-col'>
                                    ...
                                    <select value={pet.gender} onChange={(e) => setPet({ ...pet, gender: e.target.value })} class="rounded-lg px-4 py-2 w-full border-2 border-secondary2 text-lg text-primary">
                                ...
                                <div className='w-[45%] flex flex-col'>
                                    ...
                                    <input
                                        className="input rounded-lg px-4 py-2 border-2 border-secondary2 text-md text-primary"
                                        placeholder='Age'
